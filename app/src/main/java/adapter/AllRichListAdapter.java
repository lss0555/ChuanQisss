...
public class AllRichListAdapter extends BaseAdapter{
	private Context context;
	private ArrayList<RichList> mDate;
	private String type;
	private ViewHold viewHold;
	public AllRichListAdapter(Context context, ArrayList<RichList> mDate) {
		...
		viewHold.RtlType.setVisibility(View.GONE);
		  ...
		  viewHold.TvTuDiNum.setText("徒弟："+mDate.get(position).getTdcount()+"名");
		   if(mDate.get(position).getJine()==null ||mDate.get(position).getJine().equals("")){
			   viewHold.TvYesIncome.setText("总收入:0.00元");
		   }else {
					   viewHold.TvYesIncome.setText("总收入:"+mDate.get(position).getJine()+"元");
		   }
